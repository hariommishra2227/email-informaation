...
def test_connection_panel_uses_top_level_login_link(monkeypatch) -> None:
    """Microsoft login must be a normal link button, not embedded HTML or iframe."""
    page = _load_outlook_page()
    rendered = {"link_button": None, "markdown": []}
    ...
    monkeypatch.setattr(page.graph_auth, "token_exists", lambda: False)
    monkeypatch.setattr(page.graph_auth, "granted_scopes", lambda: [])
    monkeypatch.setattr(page.graph_auth, "create_login_url", lambda: "https://login.microsoftonline.com/auth")
    ...
    class FakeStreamlit:
        ...
        @staticmethod
        def link_button(*args, **kwargs):
            rendered["link_button"] = {"args": args, "kwargs": kwargs}
            return None

        @staticmethod
        def markdown(*args, **kwargs):
            rendered["markdown"].append((args, kwargs))
            return None
    ...
    assert not page._render_connection_panel()
    assert rendered["link_button"] is not None
    assert rendered["link_button"]["args"][:2] == (
        "Sign in with Outlook",
        "https://login.microsoftonline.com/auth",
    )
    assert rendered["link_button"]["kwargs"]["icon"] == "🔐"
    assert all("iframe" not in str(args).lower() for args, _kwargs in rendered["markdown"])
    assert all("meta http-equiv" not in str(args).lower() for args, _kwargs in rendered["markdown"])
