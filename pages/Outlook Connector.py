...
def _render_enterprise_sync(user_id: str) -> None:
    ...
    if not st.button("Sync Complete Mailbox", use_container_width=True):
        _render_sync_result()
        return
    ...
    result = MailboxSynchronizer(batch_size=100).synchronize(user_id, progress=update_progress)
    progress_bar.progress(1.0)
    st.session_state["enterprise_sync_summary"] = result.to_dict()
    _render_sync_result()
